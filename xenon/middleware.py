...
class LoginProtection(object):
    def __init__(self, get_response):
        ...
        self.api_key = apps.get_app_config('xenon').SHOPIFY_API_KEY
        self.api_secret = apps.get_app_config('xenon').SHOPIFY_API_SECRET
    ...
    def __call__(self, request):
        if hasattr(request, 'session') and 'shopify' in request.session and request.session.get('shopify') is not None:
